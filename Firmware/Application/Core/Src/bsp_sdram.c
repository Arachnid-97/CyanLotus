...
/* Step 3 --------------------------------------------------------------------*/
  ...
  while((FMC_GetFlagStatus(FMC_BANK_SDRAM, FMC_FLAG_Busy) != RESET) && (timeout > 0))
  {
    timeout--;
  }
  /* 发送上述命令*/
...
/* Step 8 --------------------------------------------------------------------*/
  ...
  /* 64ms/8192=7.813us  (7.813 us x FSDCLK) - 20 = 636 */
...
void SDRAM_WriteBuffer(uint32_t* pBuffer, uint32_t uwWriteAddress, uint32_t uwBufferSize)
{
  ...
  /* 禁止写保护 */
  FMC_SDRAMWriteProtectionConfig(FMC_BANK_SDRAM, DISABLE);
...
uint8_t SDRAM_Test(void)
{
  ...
  __IO uint8_t *addr = (__IO uint8_t*) (SDRAM_BANK_ADDR);
  /*按8位格式读写数据，并校验*/
  
  /* 把SDRAM数据全部重置为0 ，IS42S16400J_SIZE是以8位为单位的 */
  for (counter = 0x00; counter < SDRAM_CAPACITY; counter++)
  {
    *(__IO uint8_t*) (addr + counter) = (uint8_t)0x0;
  }

  /* 向整个SDRAM写入数据  8位 */
  for (counter = 0; counter < SDRAM_CAPACITY; counter++)
  {
    *(__IO uint8_t*) (addr + counter) = (uint8_t)(ubWritedata_8b + counter);
  }
  
  /* 读取 SDRAM 数据并检测*/
  for(counter = 0; counter<SDRAM_CAPACITY;counter++ )
  {
    ubReaddata_8b = *(__IO uint8_t*)(addr + counter);  //从该地址读出数据
  ...
  /* 把SDRAM数据全部重置为0 */
  for (counter = 0x00; counter < SDRAM_CAPACITY/2; counter++)
  {
    *(__IO uint16_t*) (addr + 2*counter) = (uint16_t)0x00;
  }
  
  /* 向整个SDRAM写入数据  16位 */
  for (counter = 0; counter < SDRAM_CAPACITY/2; counter++)
  {
    *(__IO uint16_t*) (addr + 2*counter) = (uint16_t)(uhWritedata_16b + counter);
  }
  
    /* 读取 SDRAM 数据并检测*/
  for(counter = 0; counter<SDRAM_CAPACITY/2;counter++ )
  {
    uhReaddata_16b = *(__IO uint16_t*)(addr + 2*counter);  //从该地址读出数据
