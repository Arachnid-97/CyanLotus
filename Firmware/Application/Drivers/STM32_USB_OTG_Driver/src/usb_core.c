...
// taken from https://github.com/ARMmbed/mbed-os/blob/master/cmsis/CMSIS_5/CMSIS/TARGET_CORTEX_M/Include/cmsis_compiler.h
#ifdef __GNUC__
/* compiler GCC */
                                ...
                                uint16_t            len)
{
  ...
  if (pdev->cfg.dma_enable == 0)
  {
    ...
    {
      // USB_OTG_WRITE_REG32( fifo, *((__packed uint32_t *)src) ); // warning:'packed' attribute ignored for type 'uint32_t *' {aka 'long unsigned int *'} [-Wattributes]
      USB_OTG_WRITE_REG32( fifo, __UNALIGNED_UINT32_READ(src) ); // fix __packed attribute error for some compilers
      src+=4;
    }
  }
  return status;
                         ...
                         uint16_t len)
{
  ...
  for( i = 0; i < count32b; i++)
  {
    // *(__packed uint32_t *)dest = USB_OTG_READ_REG32(fifo); // warning:'packed' attribute ignored for type 'uint32_t *' {aka 'long unsigned int *'} [-Wattributes]
    __UNALIGNED_UINT32_WRITE(dest, USB_OTG_READ_REG32(fifo)); // fix __packed attribute error for some compilers
    dest += 4 ;
  }
  return ((void *)dest);
}
